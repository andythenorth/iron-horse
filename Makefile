...
# we depend on a single static target rather than having a list of all the graphics pre-requisites
# a pre-requisites list could actually be generated by python, but it's not proven necessary so far
# !! currently graphics runs once for all grfs, this might need to change to a per-grf basis, and also add a static pre-requisite for each grf
	$(_V) $(PYTHON3) src/render_graphics.py $(PY_GLOBAL_ARGS) --grf-name=iron-moose
