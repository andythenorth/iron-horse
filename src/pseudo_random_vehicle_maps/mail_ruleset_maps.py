...
def generate_mail_map(length: int, seed: int) -> list[int]:
    if length == 1:
        return [[VALUE_FIRST, VALUE_LAST, VALUE_SPECIAL][seed % 3]]
...
def validate_map(length: int, map_values: list[int]) -> bool:
    ...
    valid_values = {VALUE_FIRST, VALUE_LAST, VALUE_SPECIAL}

    if not all(value in valid_values for value in map_values):
        return False

    # at least 2 of 3 types must be present
    if length >= 3:
        present = {
            value
            for value in (VALUE_FIRST, VALUE_LAST, VALUE_SPECIAL)
