...
class EngineConsist(Consist):
    ...
    def __init__(self, **kwargs):
        ...
        # but for engines set the multiplier to 3 by default, resetting the base cost nerf
        self.floating_run_cost_multiplier = 3
        # fixed (baseline) run costs on this subtype
        self.fixed_run_cost_points = 60
    ...
    def running_cost(self):
        ...
        speed_cost_factor = self.speed / 20
        # max power 10000hp by design - see assert_power()
        # multiplier for power, max value will be 10
        power_factor = self.power / 1000
        # bonus for electric engines, ~20% lower power costs
        # !! this is an abuse of requires_electric_rails, but it's _probably_ fine :P
        if self.requires_electric_rails:
            power_factor = 0.8 * power_factor
        # basic cost from speed, power
        floating_run_cost_points = speed_cost_factor * power_factor
        ...
        # add floating cost to the fixed (baseline) cost (which is arbitrarily up to 100 points)
        run_cost_points = self.fixed_run_cost_points + floating_run_cost_points
...
class PassengerEngineConsist(EngineConsist):
    ...
    def __init__(self, **kwargs):
        ...
        self.floating_run_cost_multiplier = 2
        # up to 100 points for fixed (baseline) run costs on this subtype
        self.fixed_run_cost_points = 90
...
class PassengerVeryHighSpeedCabEngineConsist(PassengerEngineConsist):
    ...
    def __init__(self, **kwargs):
        ...
        # fixed (baseline) run costs on this subtype
        self.fixed_run_cost_points = 150
        """
...
class MetalCarConsist(CarConsist):
    ...
    def __init__(self, **kwargs):
        ...
        self.loading_speed_multiplier = 2
        self.buy_cost_adjustment_factor = 1.2
        self.floating_run_cost_multiplier = 1.25
...
class PassengerLuxuryCarConsist(PassengerCarConsistBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.floating_run_cost_multiplier = 1.8
        # I'd prefer @property, but it was TMWFTLB to replace instances of weight_factor with _weight_factor for the default value
