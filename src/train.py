...
class EngineConsist(Consist):
    ...
    def __init__(self, **kwargs):
        ...
        self.caboose_families = {'caboose_car': 'pony_auto_1', 'goods_caboose_car': 'pony_auto_1'}
        # caboose families can be over-ridden on a per engine, per caboose type basis, keys will be replaced
        for key, value in kwargs.get('force_caboose_families', {}).items():
            self.caboose_families[key] = value
...
class CabooseCarConsist(CabooseCarConsistBase):
    ...
    """
    rows_for_auto_mapping = {
        # mappings by generation in order, number of entries must match number of generations per base track type for the roster
        "pony" = {
            "RAIL": [],
            "NG": [],
        },
