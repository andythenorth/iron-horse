...
class EngineConsist(Consist):
    ...
    def __init__(self, **kwargs):
        ...
        # hide tail lights for on engines for 2.0.0 release, possibly do dedicated engine tail lights later, possibly not
        self.hide_tail_light = True # probably move to a string or something later?
...
class Train(object):
    ...
    @property
    def tail_light(self):
        # fetch spritesheet name to use for tail light
        # !! for 2.0.0 release, option to suppress tail lights on engines, draw correct engine lights later??
        if hasattr(self.consist, 'hide_tail_light'):
            return 'empty'
        else:
