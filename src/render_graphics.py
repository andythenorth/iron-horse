...
def run_spritelayer_cargo_set_pipelines(
    ...
    for pipeline in spritelayer_cargo_set_pair[0].pipelines:
        pipeline.render(
...
def main():
    ...
    # tmp dirs used for file comparison of output, which is used to only write over the actual graphics file if it has changed
    # otherwise the nml sprite cache is undesirably invalidated for files where only the timestamp has changed
    for tmp_dir_parent in ["", "spritelayer_cargos"]:
